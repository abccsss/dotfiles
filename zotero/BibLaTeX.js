{
	"translatorID": "1fa7fda3-e8da-446e-be10-b322d0fc2ae1",
	"translatorType": 2,
	"label": "BibLaTeX",
	"creator": "Chenjing Bu",
	"target": "bib",
	...
	"lastUpdated": "2024-07-10 00:00:00"
}
...
function writeField(field, value, isMacro, noEscape) {
	...
	var fieldPadding = fieldLength < 12 ? 12 - fieldLength : 0;

	Zotero.write(",\n\t" + field + " ".repeat(fieldPadding) + " = ");
	...
	if (!noEscape && !isMacro && !(field == "url" || field == "doi" || field == "file" || field == "lccn")) {
		...
		if (field != "pages" && field != "eprintclass") {
	...
	while (item = Zotero.nextItem()) {
		...
		if (item.publicationTitle) {
			if (item.itemType == "bookSection" || item.itemType == "conferencePaper" || item.itemType == "dictionaryEntry" || item.itemType == "encyclopediaArticle") {
				...
				writeField("journaltitle", item.journalAbbreviation || item.publicationTitle);
			}
		}
		...
		if (item.date) {
			var dateLabel = item.date.includes("-") ? "date" : "year";
			writeField(dateLabel, Zotero.Utilities.strToISO(item.date));
		}
